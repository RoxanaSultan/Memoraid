...
import com.roxanasultan.memoraid.databinding.FragmentAccountCaretakerBinding
...
import com.roxanasultan.memoraid.viewmodel.AccountCaretakerViewModel
...
class AccountCaretakerFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAccountCaretakerBinding.inflate(inflater, container, false)
...
//                .circleCrop()
    ...
    private fun saveUpdates() {
        ...
        lifecycleScope.launch {
            if (username != accountViewModel.user.value?.username) {
                if (validateUsername(username)) {
                    didAnythingChange = true
                }
            }
            if (email != accountViewModel.user.value?.email) {
                if (validateEmailAddress(email)) {
                    didAnythingChange = true
                    isEmailChanged = true
                }
            }
            if (firstName != accountViewModel.user.value?.firstName) {
                didAnythingChange = true
            }
            if (lastName != accountViewModel.user.value?.lastName) {
                didAnythingChange = true
            }
            if (phoneNumber != accountViewModel.user.value?.phoneNumber) {
